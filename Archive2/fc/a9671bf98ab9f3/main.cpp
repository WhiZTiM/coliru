...
//    }
    ...
    // Attempting to use this single method as universal ref.
    template<typename U>
    inline std::enable_if_t<std::is_base_of<T,U>::value> push(std::shared_ptr<U>&& ptr)
    {
        ...
        // perfect forwarding the ptr in place
        std::cout << "perfect forwarding" << std::endl;
        data_queue.emplace(std::forward<U>(ptr)); // move the pointer
        data_cond.notify_one();  
    }
...
int main()
{
    ...
    auto dlsMessage = DataLoadSessionMsg(
        UtlMessage::USER_START, 4, "192.168.1.2", 0);
    dataLoadSessionQ.push(std::move(pDLSMessage)); // will call push(const std::shared_ptr<U>&)
    dataLoadSessionQ.push(dlsMessage); // will call push(const std::shared_ptr<U>&)
    while (!dataLoadSessionQ.empty()) {
        std::cout << dataLoadSessionQ.wait_and_pop()->getMessageSubType() << std::endl;   
    }
}
