...
struct MB_BPMessage {
    ...
    std::string key() const { return boost::lexical_cast<std::string>(boost::make_tuple(some,common,fields)); }
...
void test_roundtrip(From const& from) {
...

    {
        ...
        std::cout << "To:   " << boost::typeindex::type_id_with_cvr<To  >().pretty_name() << ", key: " << to  .key() << "\n";
        std::cout << "--\n";
    }
}
