...
uint64_t std_bitset_get_count(uint32_t number)
{
    std::bitset<8*sizeof(number)> bset(number);
    return bset.count();
}

uint64_t accumulate_true_bits_count4(uint64_t acc, uint32_t number)
{
    return acc + std_bitset_get_count(number);
}
...
int main()
{
    ...
    const uint32_t test_rounds = 5;
    input_vec.resize(500000);
    ...
    r = test_accumulator(input_vec, accumulate_true_bits_count4, test_rounds);
    printf("accum4 bits_count: %lu, took: %lu\n", r.bits_count, r.time);
    r = test_cycle(input_vec, accumulate_true_bits_count4, test_rounds);
    printf("cycle4 bits_count: %lu, took: %lu\n", r.bits_count, r.time);
