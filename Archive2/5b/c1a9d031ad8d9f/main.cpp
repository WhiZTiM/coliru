...
struct lazy_parse_impl
{
    template < typename Range, typename Parser, typename Skipper>
    any_value operator()(Range range, const Parser& parser, const Skipper& skipper) const
    {
        any_value parsed_value;
        typename Range::const_iterator begin = range.begin(), end=range.end();
        phrase_parse(begin,end,parser,skipper,parsed_value);
        return parsed_value;
    }
};
...
private:
  ...
  px::function<lazy_parse_impl> lazy_parse;
};
