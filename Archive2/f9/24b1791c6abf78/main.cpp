...
template<typename T, typename Container = std::priority_queue<std::shared_ptr<T>, std::vector<std::shared_ptr<T>>, ptr_less<std::shared_ptr<T>>>>
...
public:
    ...
    inline std::shared_ptr<T> wait_and_pop() {
        ...
        if (!shutdownFlag) {
            auto res = std::move(data_queue.top());
            data_queue.pop();
            return std::move(res);
        }
        return nullptr;
    ...
    inline bool try_pop(std::shared_ptr<T>& rValue) {
        ...
        if (data_queue.empty()) {
            ...
            rValue = std::move(data_queue.top());
            data_queue.pop();
            return true;
        }
    }
