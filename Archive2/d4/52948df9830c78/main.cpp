...
// HACK for demo:
namespace std {
    static inline std::ostream& operator<<(std::ostream& os, std::vector<char> const& v) {
        os << "vector<char> { \"";
        os.write(&v[0], v.size());
        return os << "\" }";
    }
...
void run_test(Target into) {
    ...
    bool ok = qi::parse (first, last, split_string, into);

    std::cout << "Parsed: " << ok << "\n";

    if (ok) {
        std::cout << fus::as_vector(into) << '\n';
    } else {
