...
#define WRAP_FUNCTION(name, enterLogging, exitLogging) 														\
		template <typename ...Args>																			\
		auto name(Args&&... args) -> decltype(std::declval<WrappedType>().name(args...))					\
		{																									\
			updateCaller();																					\
																											\
			struct dummy																					\
...
#define WRAP_RW_MEMBER(name) decltype(VALUE.name)& name = VALUE.name;
#define WRAP_RO_MEMBER(name) const decltype(VALUE.name)& name = VALUE.name;	


#define WRAP_ASSIGMENT_OP(enterLogging, exitLogging)																	\
	template <typename T, typename = typename void_t<decltype(std::declval<WrappedType&>() = std::declval<T>())>::type>	\
	DebugWrapper& operator=(T value)																					\
	{																													\
		updateCaller();																									\
																														\
		enterLogging;																									\
...
#undef WRAP_FUNCTION
#undef WRAP_RW_MEMBER
#undef WRAP_RO_MEMBER
#undef WRAP_ASSIGMENT_OP
