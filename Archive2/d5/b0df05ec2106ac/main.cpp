...
//
//#include <assert.h>
#include "/Archive2/4e/e8fc626df40d51/main.cpp"
#include <limits.h>
...
struct DTree{
    ...
    const Node *searchfi(const inType &that, bool (*comp2)(const T &, const inType &) = &_less, bool (*equalcomp)(const T &, const inType &) = &_equal) const{
        //can be empty
        const Node *it = root();
        DIR dir;
        while (1){
    ...
    template <typename inType>
    Node *searchfi(const inType &that, bool (*comp2)(const T &, const inType &) = &_less, bool (*equalcomp)(const T &, const inType &) = &_equal) {
    ...
    template <typename inType>
    const Node *find(const inType &ite) const{
        return searchfi(ite);
    }
    
    template <typename inType>
    Node *find(const inType &ite) {
        return const_cast<Node *>(cons().find(ite));
    }
