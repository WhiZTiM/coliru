...
//=======================================
    ...
    template<class... Args>
      void print1(const std::tuple<Args...>& t) 
      {
        std::cout << __PRETTY_FUNCTION__ << "(";
        TuplePrinter<decltype(t), sizeof...(Args)>::print(t);
	    std::cout << ")\n";
    ...
    template<class T>  static void print1( T&& obj ) { std::cout << "{" << __PRETTY_FUNCTION__ << "}" << obj; }

    template<class T>  static void print1( std::unique_ptr<T>& obj ) { std::cout << "{" << __PRETTY_FUNCTION__ << "}" << *obj; }
