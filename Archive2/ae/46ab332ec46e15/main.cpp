...
int main()
{
	...
	for (std::size_t i(0); i < cloud.size(); ++i) if (!std::binary_search(rec.begin(), rec.end(), i))
	{     
		std::cout << "neighborhood of point " << cloud[i] << "\n---------------------\n\n";

        std::for_each(rtree.qbegin(bgi::satisfies([&p = cloud[i]](pointI const& qP) {
		    auto d = boost::geometry::distance(p.first, qP.first);
			return d && d < 5;
