...
void ByPopularDemand(std::vector<int> source)
{
    ...
    odds.reserve(source.size());
    evens.reserve(source.size());
    
    std::copy_if(source.begin(), source.end(), std::back_inserter(odds), [](int x) { return x % 2 == 1; });
    std::copy_if(source.begin(), source.end(), std::back_inserter(evens), [](int x) { return x % 2 == 0; });
