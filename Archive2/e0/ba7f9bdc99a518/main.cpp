...
struct Map
{
    ...
    struct iterator
    {
        ...
        iterator(FirstIt first, SecondIt second) : firstIt(first), secondIt(second), mPair(*first, *second)
        {
        ...
        iterator& operator++()
        {
            return *this = iterator(++firstIt, ++secondIt);
        }
...
int main()
{
    ...
    for (auto& el : map)
    {
        std::cout << std::flush << el.first << '-' << el.second << std::endl;
    }

    for (auto i = map.begin(), e = map.end(); i != e; ++i)
    {
        std::cout << std::flush << i->first << '-' << i->second << std::endl;
    }
}
