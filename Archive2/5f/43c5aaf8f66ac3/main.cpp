...
namespace {
  ...
  inline size_v<10> count(hierarchy<10> const &) { return {}; }
  
  template<typename T>
  struct wrapper {
    template<typename U = T>
    friend inline size_v<T::value> count(identity_t<U> const &) { return {}; }
  };

  template<
    typename T = hierarchy<0>,
    typename B = decltype(count(T{})),
    typename   = decltype(wrapper<hierarchy<B() ? B() - 1 : 0>>{})
  >
  constexpr size_t f() {
    return B();
  }
