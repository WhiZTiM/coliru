...
// GCC supports expression SFINAE
//template<class Q>
...
// MSVC does not support expression SFINAE - must overload on std::queue
template<class T>
auto front_or_top(std::queue<T>& q) -> decltype(q.front()) {
    return q.front();
}

// MSVC does not support expression SFINAE - must overload on std::priority_queue
template<class T>
auto front_or_top(std::priority_queue<T>& q) -> decltype(q.front()) {
    return q.top();
}
...
{    
    ...
    UtlThreadSafeQueue<PriorityLevel, std::priority_queue<
        ...
        ptr_less<std::shared_ptr<PriorityLevel>>>> priorityIPCQueue1;
    ...
    for (int i = 0; i < 3; i++) {
        // launch 3 priority threads
        auto fut = std::async(std::launch::async, [](decltype(priorityIPCQueue)& rJobQueue) {
        ...
        priorityFutures.push_back(std::move(fut));
    }
    // kick off 3 parallel threads to work on the shared job queue
    std::vector<std::future<void>> normalFutures;
    for (int i = 0; i < 3; i++) {
        // launch 3 priority threads
        auto fut = std::async(std::launch::async, [](decltype(priorityIPCQueue)& rJobQueue) {
            while (!rJobQueue.empty()) {
        ...
        normalFutures.push_back(std::move(fut));
    }
    
    // now that the threads were launched asynchronously
    // let them each perform their work (each thead takes 1 sec)
    std::this_thread::sleep_for(std::chrono::seconds(3));
    
    std::for_each(priorityFutures.begin(), priorityFutures.end(), [](std::future<void>& fut) {
    ...
    std::for_each(normalFutures.begin(), normalFutures.end(), [](std::future<void>& fut) {
        // this kicks off the threads
