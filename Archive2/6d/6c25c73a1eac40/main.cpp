...
struct ArenaAllocator {
...
#if !defined _MSC_VER    
    // libstdc++ attempts to default-construct the allocator:
    // __a == _Alloc()
    ArenaAllocator() : m_arena(nullptr) {}

    // The following definitions are required by libstdc++.
    // allocator_traits just do not work here because it's broken.
    using size_type = typename std::allocator<T>::size_type;
    using difference_type = typename std::allocator<T>::difference_type;
    using pointer = typename std::allocator<T>::pointer;
    using const_pointer = typename std::allocator<T>::const_pointer;
    // Note that "reference" is not even in Allocator Requirements.
    ...
    // required by both VC++ and libstdc++
    template<class U, class... Args> void construct(U* p, Args&&... args) { Traits::construct(*this, p, std::forward<Args>(args)...); }
    template<class U> void destroy(U* p) { Traits::destroy(*this, p); }
...
#if 0 // The following do not work in both VC++ and g++
