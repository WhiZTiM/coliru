...
template<typename T>
struct is_transparent<T, void_t<decltype(T::is_transparent)>> : std::true_type {};
}
...
public:
    ...
    template <typename K, typename C1 = Comparator, typename = typename std::enable_if<wtf::is_transparent<C1>::value, void>::type>
...
int main(){
   ...
   flatmap<std::string, int, std::less<>> fm, fm2;
   swap(fm, fm2);
