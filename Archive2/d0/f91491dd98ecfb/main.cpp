...
struct controller
{
    ...
    // for instance (only for exposition):
...
struct view : view_base
{
    ...
    // retrive the information from the model and display it
    ...
    // TODO: if the view accepts user input requesting a change in the data
    // forward the request to the associated controller

    protected:
        model& model_ ; // and to the model
        controller& controller_ ; // the view holds a reference to its controller
};
...
int main()
{
    ...
    controller.say_hello_to_all() ; // only for exposition
}
