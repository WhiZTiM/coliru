...
public:
	...
	bool awaitTermination(std::size_t timeout = 0) {
		if (timeout == 0) {
			...
			{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> if (timeout == 0) {" << '\n';}
			return true;
		} else {
			try {
			...
			if (poolSize_ == 0) {
				{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> } else { --> return true;" << '\n';}
				return true;
			} else {
				{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> } else { --> return false;" << '\n';}
				return false;
			}
		}
	}
	...
	void execute(void (*command) (void *), void *arg) {
		if (shutdown_) {
			{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> if (shutdown_) {" << '\n';}
			return;
		}

		if (poolSize_ < corePoolSize_) {
			{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> if (poolSize_ < corePoolSize_) { --> " << "poolSize = " << poolSize_ << '\n';}
			createThread();
		} else if (poolSize_ < maximumPoolSize_ && taskQueueSize_/(poolSize_.load() > 0 ? poolSize_.load() : 1) > threshold_) {
			{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> } else if (poolSize_ < maximumPoolSize_ && taskQueueSize_/poolSize_ > threshold_) { --> " << "poolSize = " << poolSize_ << " --> taskQueueSize_/poolSize_ = " << taskQueueSize_/poolSize_ << '\n';}
			createThread();
		}
		...
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> conditionVariable_.notify_one();" << '\n';}
	...
	void prestartAllCoreThreads() {
		while (poolSize_ < corePoolSize_) {
			{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> " << "poolSize = " << poolSize_ << '\n';}
			createThread();
		}
	}
...
protected:
	...
	inline void eh(const char* what) { 
		try {
			...
			{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> catch(...)" << '\n';}
		}
	}
	...
	void startRoutine() {
		std::size_t i;{std::lock_guard<std::mutex> lock(stdCoutMutex_);++threadNo;i=threadNo;std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "start" << '\n';}
		...
		while (1) {
			try {
				do {
					...
					if (timerEnabled) {
						...
						{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "conditionVariable_.wait_for.woken up or timeout" << '\n';}
					} else {
						conditionVariable_.wait(lk);
						{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "conditionVariable_.wait.woken up" << '\n';}
					}
				} while(0);

				if (shutdownNow_) {
					{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (shutdownNow_) { --> outer 1" << '\n';}
					break;
				}
				...
				while (1) {
					if (taskQueue_.pop(t)) {
						...
						t.command(t.arg);
						++ completedTaskCount_;
						{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (taskQueue_.pop(t)) {" << '\n';}
					} else {
						{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "the queue is empty. std::lock_guard<std::mutex> lock(terminateTheThreadMutex_);" << '\n';}
						...
						if (poolSize_ > corePoolSize_) {
							if (!timerEnabled) {
								...
								{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (!timerEnabled) {" << '\n';}
							} else {
								if (cvs == std::cv_status::timeout) {
									terminateTheThread = true;
									{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "terminateTheThread = true;" << '\n';}
								} else {
					...
					if (shutdownNow_) {
						{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (shutdownNow_) { --> inner" << '\n';}
						break;
					}
				}
			} catch(const std::exception& e) {
			...
			if (shutdown_) {
				{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (shutdown_) {" << '\n';}
				break;
			}

			if (shutdownNow_) {
				{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (shutdownNow_) { --> outer 2" << '\n';}
				break;
			}

			if (terminateTheThread) {
				{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "if (terminateTheThread) {" << '\n';}
				break;
			}
		}
		-- poolSize_;
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> thread no. = " << i << " --> " << "exit --> poolSize = " << poolSize_ << '\n';}
	}
...
protected:
	...
	virtual void exceptionHandling(const char* what) {
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> exception = " << what << '\n';}
		throw Size();
	}
};
...
int main() {
	...
	do {
		MyThreadPoolExecutor tpe(123456);
/*
...
//		const long SIZE = 3000000;
	...
	//	tpe.execute(throw_size, &in);
		...
		//		tpe.shutdownNow();
				...
				// Clean up.
				{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> Clean up." << '\n';}
		...
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> max threadNo = " << threadNo << '\n';}
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> time = " << difference.count() << " s\n";}
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> cnt = " << cnt << '\n';}
		{std::lock_guard<std::mutex> lock(stdCoutMutex_);std::cout << "In function " << __func__ << " --> CompletedTaskCount = " << tpe.getCompletedTaskCount() << '\n';}
	} while(0);
