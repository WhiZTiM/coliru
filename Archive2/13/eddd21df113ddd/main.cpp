...
struct grammar: qi::grammar<It, data (), Skipper>
{
  grammar () : grammar::base_type (start)
  {
    ...
    static auto const set_value_impl = [] (data& d, any_value const& value)
    { d.value = value; };
    
    static px::function<decltype (set_value_impl)> const 
      set_value { set_value_impl };
    ...
    parsed_type_ %=  raw[value_selector [ _r1  = _1 ]]
      > omit [ -(  // if the range is not empty, parse it with _r1 grammar
          eps (_r2) 
        >> custom::parse_from_range (_r2) [ lazy (*_r1) [set_value (_r3, _1) ] ]
      ) ]
    ;

    // if _r1 is not NULL then parse with it; otherwise parse as json
    value_ = ( eps (_r1) >> lazy (*_r1) [ set_value (_r3, _1) ]) | parse_as_json_ (_r2);
    
    data_
      %= eps [ _rule = nullptr ]
      > '{' > (
          kwd ( lit ("\"type\"" ) ) [ ':' > parsed_type_ (_rule, _range, _val) > ',' ]
        / kwd ( lit ("\"value\"") ) [ ':' > value_ (_rule, _range, _val)       > ',' ]
      ) > '}'
     ;
...
private:
  ...
  qi::rule<It, std::string (value_rule_type *&, range_type&, data&), Skipper> parsed_type_;
  qi::rule<It, any_value   (value_rule_type *&, range_type&, data&), Skipper> value_;
...
static std::string const sample6 = R"(
{
  "value": [ 111, 333 ],
  "type": "int2b",
})";
