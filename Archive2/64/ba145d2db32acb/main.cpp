...
struct decorator_wrapper : decorator_wrapper_helper<Source, but_last_t<Decorators...>>
                         , public last_t<Decorators...>
{
    using first_base_type = decorator_wrapper_helper<Source, but_last_t<Decorators...>>;
    using second_base_type = last_t<Decorators...>;
    
    template <typename... Args>
	decorator_wrapper(Args&&... args)
		: first_base_type(std::forward<Args>(args)...)
		, second_base_type(std::get<sizeof...(Decorators)-2>(first_base_type::_tuple))
	{
	}
};
