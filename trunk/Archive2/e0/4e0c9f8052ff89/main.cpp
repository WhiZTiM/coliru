    ...
    void parce(const string_t &str){
        ...
        while ((parens.top().*(parens.top().parceFunction()))(it, str, parens)){
            if (isOpenParenthesis(*it)){
                erint "opening parenthesis";
                ++parens = Parenthesis();
            }
            else if (isCloseParenthesis(*it)){
                 erint "closing parenthesis";
                if (parens.size() != 1){
                //    erint "inhither";
        ...
        for (Parenthesis *pit=&parens.top(); pit!=&parens[-1]; --pit){
                if (pit->parceFunction() == &Parenthesis::secondaryParce){
                      // erint "p2";
                }
                else {
                    //erint "p1";
                    assert(pit->parceFunction() == &Parenthesis::secondaryDeepParce);
