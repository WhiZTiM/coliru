...
auto mbind(StateL<State, A> g, F k) -> decltype(k(g(State()).front().first))
{
    return [g, k](State s) {
        ...
        //List<PList<B>> 
        auto lst2 = fmap([k](pair<A, State> p) {
            A a = p.first;
            State s1 = p.second;
            auto ka = k(a);
...
yield return future >>= [self = *this](auto x) \
{ \
    auto copy = self; \
    copy.wanted = x; \
    return copy(); \
...
#define await_void(future) \
yield return future >>= [self = *this](auto _) \
{ \
    auto copy = self; \
    return copy(); \
} \
...
auto solve()
{
    ...
    struct Coro : boost::asio::coroutine
    {
        ...
        StateL<List<int>, tuple<int,int,int>> operator()()
        {
            StateLI::apply<int>::type sel = &select<int>;
            const auto unit = MonadUnit<StateLI>{};
            const auto guard = MonadGuard<StateLI>{};
            while(true) reenter(*this)
            {
                await(s, sel);
                await(e, sel);
                await(n, sel);
                ...
                await_void(guard(s != 0 && m != 0));
                
                send  = asNumber({s, e, n, d});
                more  = asNumber({m, o, r, e});
                money = asNumber({m, o, n, e, y});
                await_void(guard(send + more == money));
                
                return unit(make_tuple(send, more, money));
            }
        }
    } coro;
    return coro();
