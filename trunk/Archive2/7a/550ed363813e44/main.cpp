...
constexpr const int CACHE_LINE_BYTES = 64;
...
private:
    ...
    std::atomic<size_t> ahead;
    T ring_[size];
    char false_share_pad[std::max(1, CACHE_LINE_BYTES - int(sizeof(size_t)) - int(size * sizeof(T)))];
    std::atomic<size_t> atail;
};
