...
#define CREATE_STEP(S,E) template<> struct Step<S, E> { \
    constexpr static bool exists = true; \
    static void doAction() { std::cout << __PRETTY_FUNCTION__ << std::endl; } \
}

CREATE_STEP(Start, Point1);
CREATE_STEP(Point1, Point2);
CREATE_STEP(Point2, End);
...
struct build_BFS_result {
    ...
    struct builder {
        ...
        using step_t = Step<prev, Current>;
...
struct build_BFS_result<S, E, All, Result, false> {
    // has no path
    using type = None;
};
...
// nothing to process
template<class S, class E, class All, class Result>
struct BFS_impl<S, E, All, type_list<>, Result> : build_BFS_result<S, E, All, Result>{
};
...
struct BFS_next {
    ...
    struct add_reachable<type_list<T1s...>, type_list<AllT...>>{
        using type = type_list<std::conditional_t<Step<T, AllT>::exists && std::is_same<T1s, None>::value, T, T1s>...>;
    };
...
: std::conditional_t<
    ...
    build_BFS_result<S, E, All, Result>> { };


template<class S, class E, class A>
using BFS = BFS_impl<S, E, A, type_list<S>, make_repeat_list<None, A::size>>;

template<class S, class E, class All>
struct Path {
    using path_t = typename BFS<S, E, All>::type;
    template<class... Ts>
    static void doAction_impl(type_list<Ts...>) {
        using expander = int[];
        (void)expander{(Ts::doAction(), 0)..., 0 };
    }
    static void doAction() {
        doAction_impl(path_t());
    }
};

int main(){
    using all_points = type_list<Start, Point1, Point2, Point3, End>;
    Path<Start, End, all_points>::doAction();
}
