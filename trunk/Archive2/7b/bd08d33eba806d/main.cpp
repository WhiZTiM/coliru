...
struct Expression{
    ...
    enum VARIABLETRAVERSESTATE{
          GOIN,
          BOTH,
          STAYOUT
    };
    static const size_t VARIABLETRAVERSESTATEnum = STAYOUT+1; 
   ...
   // BinaryOperator *(*bofn)();
    ...
    struct OperatorBase: OperatorStorage{
       ...
       // typedef Truet hellor;
        ...
        void traverse(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dir=LEFT) const {
            cons()->traversef(fn, vars, dir);
        }
        
        template <typename FN>
        void traverse(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dir=LEFT) {
            getT()->traversef(fn, vars, dir);
        }
    ...
    struct AtomicExpression: OperatorBase<AtomicExpression>{
        ...
        template <typename FN>
        void traversef(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dir=LEFT) const {
            //erint "outputting number";
            fn(*this);
        }
        
        template <typename FN>
        void traversef(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dir=LEFT) {
            fn(*this);
        }
    ...
    struct Variable: OperatorBase<Variable>{
        ...
        void traversef(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dir=LEFT) const {
            ...
            if (expression()->expressionType() == NUMBER){
                //erint "hrm";
                static_cast<AtomicExpression*>(expression())->traversef(fn, vars, dir);
                //erint "hrm2";
            }
            else if (expression()->expressionType() == BINARY) {
                 static_cast<BinaryOperator<Add>*>(expression())->traversef(fn, vars, dir);
            }
            else if (expression()->expressionType() == COMMUTATIVE){
                //commutative   
                static_cast<CommutativeOperator<Add2>*>(expression())->traversef(fn, vars, dir);
            }
            else {
                static_cast<Variable *>(expression())->traversef(fn, vars, dir);
            }
            }
            if (vars >= BOTH){
    ...
    struct BinaryOperator: OperatorBase<BinaryOperator<TrueT>> {
        ...
        template <typename FN>
        void traversef(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dirr=LEFT) const {
            //erint "traversing binary";
            DIRECTION dir = dirr;
            if (child(dir)->expressionType() == NUMBER){
                //erint "hrm";
                static_cast<AtomicExpression*>(child(dir))->traversef(fn, vars, dirr);
                //erint "hrm2";
            }
            else if (child(dir)->expressionType() == BINARY) {
                 static_cast<BinaryOperator*>(child(dir))->traversef(fn, vars, dirr);
            }
            else if (child(dir)->expressionType() == COMMUTATIVE){
                //commutative   
                static_cast<CommutativeOperator<Add2>*>(child(dir))->traversef(fn, vars, dirr);
            }
            else {
                static_cast<Variable *>(child(dir))->traversef(fn, vars, dirr);
            }
            // erint "outputting binary";
            fn(*this);
        ...
        template <typename FN>
        void traversef(FN fn, const VARIABLETRAVERSESTATE vars=GOIN, const DIRECTION dirr=LEFT) {
