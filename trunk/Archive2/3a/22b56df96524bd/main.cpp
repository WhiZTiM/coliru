...
namespace palindrome {
	...
	std::deque<substrings> annotate_graph(const std::string& s)
	{
		...
		for(std::size_t i = 0, end = s.length(); i < end; ++i)
		{
			const char vertice = s[i];

			// skip non-alphanumeric...
			if(!std::isalpha(vertice) && !std::isdigit(vertice))
			{
				continue;
			}

			const char vertice_index = vertice - char_offset;

			// see if we've visited this vertice before...
			if(! annotations[vertice_index].empty())
			{
				std::for_each(annotations[vertice_index].begin(), annotations[vertice_index].end(), [&substringsToCheck, i](const std::size_t start)
				{
			...
			annotations[vertice_index].push_back(i);
		}
