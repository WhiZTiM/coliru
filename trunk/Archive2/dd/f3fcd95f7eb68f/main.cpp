...
int main() {
    size_t length;
    std::vector<Graph> dataG = creategraphs(splitstringtolines(readfromfile("sehe2.txt", length)));

    for (auto i = 0u; i < dataG.size(); ++i)
    {
        std::ofstream ofs("graph" + std::to_string(i) + ".dot");
        write_graphviz(ofs, dataG[i], make_label_writer(get(&VertexProperties::label, dataG[i])), make_label_writer(get(&EdgeProperties::label, dataG[i])));
    }
