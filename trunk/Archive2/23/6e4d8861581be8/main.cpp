...
struct parser: qi::grammar<It, myline(), Skipper> {
  parser()
      : parser::base_type(start),separator("@") {
    using namespace qi;
    ...
    one    = (string >> qi::lit(_r1) >> qi::int_) [_val           = phx::construct<myline>(_2, _1)];
    two    = (qi::int_ >> qi::lit(_r1) >> string);


    keyword.add("one", &one)("two", &two);

    // NOTE the %=
    line %= omit [ keyword[_a = _1] ] >> qi::lazy(rule_with_params(*_a,separator));
...
private:
  ...
  typedef qi::rule<It, myline(std::string), Skipper> RuleInhAttr;

  Rule<myline> start;
  RuleInhAttr one, two;
  qi::rule<It, myline(), Skipper, qi::locals<RuleInhAttr* > > line;

  Rule<std::string> string;

  qi::symbols<char, RuleInhAttr* > keyword;
