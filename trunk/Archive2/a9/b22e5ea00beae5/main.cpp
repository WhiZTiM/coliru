...
{
    os << '<';
    if (!container.empty()) os << (*vector.cbegin());
    for (C::const_iterator it = ++container.cbegin(); i != container.cend(); ++it) os << ',' << (*it);
    os << '>';
    return os;
}

template<typename K, typename V>
std::ostream& operator<<(std::ostream& os, const std::pair<K,V>& container)
{
...
int main()
{
    ...
    std::cout << vect2.at(2) << std::endl;
    std::map<std::string, int> map = {{"a", 1}, {"b", 2}};
    attrVect = {};
    AttrValue attrTest = {map, 1, "hola"};
