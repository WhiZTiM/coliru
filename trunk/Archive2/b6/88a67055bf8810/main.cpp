...
int main()
{
    ...
    auto t2 = std::make_tuple(6, 5, 4, 3, 2, 0);
    ...
    std::size_t t2hash = hasher(t2);
    
    std::size_t t0hsah = hasher(reverse(t0));
    std::size_t t1hsah = hasher(reverse(t1));
    std::size_t t2hsah = hasher(reverse(t2));

    using hash_tuple::hash_combine;
    std::size_t cmb01 = hash_combine(t0hash, t1hash);
    std::size_t cmb02 = hash_combine(t0hash, t2hash);
    std::size_t cmb10 = hash_combine(t1hsah, t0hsah);
    std::size_t cmb20 = hash_combine(t2hsah, t0hsah);

    std::cout << std::boolalpha;
    std::cout << cmb01 << " == " << cmb10 << " -> tuple_0 == reverse(tuple_1) = " << (cmb01 == cmb10) << std::endl;
    std::cout << cmb02 << " == " << cmb20 << " -> tuple_0 == reverse(tuple_2) = " << (cmb02 == cmb20) << std::endl;
