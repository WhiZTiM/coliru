...
int main()
{
    ...
    /*auto x = */
    test(
        merge_tuple(std::tuple<std::tuple<std::tuple<std::tuple<Noisy, Noisy>>>, std::tuple<std::tuple<Noisy>>>(/*std::make_tuple(std::make_tuple(std::forward_as_tuple(std::move(n), n))), std::tuple<std::tuple<Noisy>>())*/)
    );
    std::cout << "----" << std::endl;
