...
void spirit_symbols(std::vector<std::string>& result, const std::string& input, char const* delimiter)
{
	result.clear();

    using namespace bsq;
    symbols<char, unused_type> charset;

    for(auto ch = delimiter; *ch; ++ch)
        charset += ch;

    if (!parse(input.begin(), input.end(), raw[*(char_ - charset)] % charset, result))
        result.push_back(input);
}
...
namespace detail {
    void make_delim_charset(char* delimiter) {
        char* s = delimiter;
        char* e = s + strlen(s);
        std::sort(s, e, [](char a, char b) { return (a!=b && a=='-') || (b!='-' && a<b); });
        *std::unique(s, e) = 0;
    }
}
...
int main()
{
    ...
    detail::make_delim_charset(separator);
