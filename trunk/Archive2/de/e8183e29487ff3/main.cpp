    ...
    -> typename std::enable_if<argIdx == 1>::type
{
    ...
    ref.append(typeid(typename fun::template arg<0>).name()).append(" ");
}
...
{
    using fun = function_traits<std::function<R(Args...)> >;
    ref.append(typeid(typename fun::template arg<argIdx-1>).name()).append(" ");
...
{
    std::string s;
    getArgTypes<F::nargs, typename F::result_type, typename F::template arg<Is>...>(s);
