...
private:
    ...
    std::unique_ptr<AttrValueBase> ptr;
    
public:
    AttrValue() : ptr(nullptr) {}
    
    AttrValue(const AttrValue&  value) : ptr(value.ptr) { std::cout << "AttrValue(const AttrValue&  value)" << std::endl; }
    ...
    AttrValue& operator=(AttrValue&& value) {
        ...
        std::swar(ptr, value.ptr);
        return *this;
    }
    
    AttrValue& operator[](size_t pos) { assert(type() == AttrType::LIST); return value<List>()[pos]; }
