...
struct DerivedMessage : public BaseMessage {
    DerivedMessage(const int value) 
        : BaseMessage()
        , mValue(value) 
    {}
    
    virtual ~DerivedMessage() = default;
    int mValue;
};
...
int main()
{
    ...
    auto message = std::make_shared<BaseMessage>();
    dataLoadSessionQ.push(std::move(message));
    auto poppedMessage = dataLoadSessionQ.wait_and_pop();
