...
// Forward your vistor implementation, in this case to handle containers based on CRTP.
...
struct visitor_base
{
    ...
    // param T. This allows us to dynamic_cast up subclass vistor<T>. Note that this pattern is 
    ...
    void visit (T & t) const
    {
        ...
        // T is deduced as T&, or T const&, but we are actually casting to visitor<T>
...
// These are the containers. When one uses the visitor pattern, there is no need to have a 
// non-template base class for type recognition. In this example, I have implemented const and 
// non-const methods. One could also implement & and && qualified methods (C++11). the appropriate
// vistor<T> will call thses methods as implemented by the user.
...
// This uses overload resolution to select the appropriate overload, which calls apis on the specfic container.
