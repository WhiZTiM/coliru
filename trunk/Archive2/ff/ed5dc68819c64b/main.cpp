...
public:
    ...
    template<Enum E>
    typename std::tuple_element<static_cast<std::size_t>(E), std::tuple<T...>>::type get()
    {
        return std::get<static_cast<std::size_t>(E)>(_tuple);
    }
    
    template<Enum E, typename... Args>
    typename std::tuple_element<static_cast<std::size_t>(E), std::tuple<T...>>::type::result_type call(Args&&... args)
    {
        return std::get<static_cast<std::size_t>(E)>(_tuple)(std::forward<Args>(args)...);
    }
};
