...
public:
    ...
    // perfect forwarding aware push with std::shared_ptr types - no need to construct new shared_ptr
    // we should be able to perfect forward from the existing one.
    template<typename U>
    inline std::enable_if_t<std::is_base_of<T, std::decay_t<U>>::value> push(std::shared_ptr<U>&& value) {
        ...
        data_queue.emplace(std::forward<U>(value));
...
int main()
{
    ...
    dataLoadSessionQ.push(pDLSMessage);
    while (!dataLoadSessionQ.empty()) {
        std::cout << dataLoadSessionQ.wait_and_pop() << std::endl;   
    }
}
