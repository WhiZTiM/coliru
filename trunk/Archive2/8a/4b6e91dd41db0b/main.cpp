...
template<typename Class,typename Type, std::size_t Offset>
struct member
{
    ...
    {
        return *reinterpret_cast<const Type*>(&obj + offset);
    }
    
    static Type& get(Class& obj)
    {
        return *reinterpret_cast<Type*>(&obj + offset);
    }
};

#define INIT_MEMBER(Class, Member, Value) (members<Class>::push<member<Class, decltype(Member),0>>(), Value)

struct foo
{
    float x   = INIT_MEMBER(foo, x,    0.0);
    float y   = INIT_MEMBER(foo, y,    0.0);
    bool flag = INIT_MEMBER(foo, flag, false);
};

int main () {
  print_type<members<foo>::value<>> ();
}