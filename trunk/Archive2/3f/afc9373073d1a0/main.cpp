...
public:
    //MatrixViaVector();
    MatrixViaVector(int m=3,int n=3);
    template <class H>
...
template <class T>
MatrixViaVector<T>::MatrixViaVector(int x,int y)//creates a m by n matrix????
{
    ...
    ostream& operator <<(ostream& outs, const MatrixViaVector<T> & obj)
    {
       ...
       for (std::vector<int>::size_type i = 0; i < obj.m; i++){
        for (std::vector<int>::size_type j = 0; j < obj.n; j++)
            outs << " "<< obj.matrix[i][j];
        outs<<endl;
    }
        outs<<endl;
