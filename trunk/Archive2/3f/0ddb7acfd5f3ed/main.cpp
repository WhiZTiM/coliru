...
namespace marshal
{
    typedef std::uint64_t max_type;

    template<std::size_t Size, typename T = int>
    struct bitsection
    {
        static_assert(sizeof(T) <= sizeof(max_type), "Type size exceeds highest allowed type size.");
    ...
    private:
        ...
        struct view
        {
            ...
            union
            {
                marshal::max_type value;
                std::array<unsigned char, byte_length> array;
            };

            static_assert(Index < sizeof...(Sections), "Index out of bit field range.");
            static_assert(byte_length <= sizeof(marshal::max_type), "Invalid type with size larger than the maximum allowed.");

            view(unsigned char* buffer)
                : buffer(buffer + byte_offset), value(0)
            {
    ...
    public:
        ...
        void set(R value)
        {
            ...
            v.value &= ~(((1 << section::size) - 1) << v.bit_shift);
            v.value |= (value & ((1 << section::size) - 1)) << v.bit_shift;
            std::copy_n(v.array.data(), v.byte_length, v.buffer);
        }
        ...
        typename section_at<Index, Sections...>::section::type get()
        {
            ...
            std::copy_n(byte_offset, byte_length, copy_array.data());
