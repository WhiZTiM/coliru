...
template<typename T>
std::ostream& operator<<(std::ostream& ostream, const Point3D<T>& point)
{
...
std::ostream& operator<<(std::ostream& ostream, const std::vector<T>& vector)
{
    ...
    ostream << vector.at(vector.size()-1)<< '}';
    return ostream;
}
...
void kmeans(std::vector<Pixel>& pixels, std::size_t numClusters) {
	...
	do {
        ...
        std::cout << iteration << ": " << clustersNew << std::endl;
		++iteration;
...
int main()
{
    std::vector<Pixel> pixels = {Pixel{5,1,1}, Pixel{7,2,2}, Pixel{3,4,1}, Pixel{4,2,6}, Pixel{5,5,1}};
    kmeans(pixels, 3);
