...
yield this->result = future >>= [self = *this](auto x) \
{ \
    auto copy = self; \
    copy.wanted = x; \
    copy(); \
...
yield this->result = future >>= [self = *this](auto _) \
{ \
    auto copy = self; \
    copy(); \
    return copy.result; \
...
auto solve()
{
    ...
    struct Coro
    {
        ...
        StateL<List<int>, tuple<int,int,int>> result;
        ...
        void operator()()
        {
            ...
            reenter(coro)
            {
                ...
                result = unit(make_tuple(send, more, money));
            }
        }
    } coro;
    coro();
